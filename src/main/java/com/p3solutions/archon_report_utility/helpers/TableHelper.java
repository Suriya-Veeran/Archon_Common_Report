...
public class TableHelper {
    ...
    public void setCellConfiguration(Table table, TableBean tableBean) {
        ...
        styleCellComponent(cell, tableBean);

        switch (tableBean.getCellInputBean().getCellConfigBean().getCellStructureType()) {
            case SINGLE_CELL:
                break;
            case CELL_WITH_HEADER_AND_VALUES:
                buildCellWithHeaderAndValues(cell, tableBean.getCellInputBean(), table);
                break;
            case CELL_WITH_JOB_STATUS:
                break;
            default:
                throw new EnumNotFound("Cell Structure Type is not invalid " + tableBean.getCellInputBean().getCellConfigBean().getCellStructureType());
        }
    }

    private static void buildCellWithHeaderAndValues(Cell cell, CellInputBean cellInputBean, Table table) {
        for (Map.Entry<String, String> entry : cellInputBean.getCellConfigBean().getHeaderAndValueParameters().entrySet()) {
            String header = entry.getKey();
            String value = entry.getValue();
            cell.add(
                            new Paragraph(new Text(header))
                                    .setFontSize(cellInputBean.getCellConfigBean().getHeaderFontSize())
                                    .setFontColor(hexaDecimalToRGB(cellInputBean.getCellConfigBean().getHeaderCellColor()))
                    )
    ...
    private static void styleCellComponent(Cell cell, TableBean tableBean) {
        ...
        CellHelper.setCellBorder(cell,
                tableBean.getCellInputBean().getBorderBean().getBorderTop(),
                tableBean.getCellInputBean().getBorderBean().getBorderBottom(),
                tableBean.getCellInputBean().getBorderBean().getBorderLeft(),
                tableBean.getCellInputBean().getBorderBean().getBorderRight());
